...
A travel packing-list app for organizing what to bring on a trip. Add items with quantities, mark them as packed, delete individual entries, and clear the list with confirmation.

Sort the list by input order, description, or packed status. A progress summary shows the number of entries packed and the completion percentage, while `localStorage` keeps the list between visits in the same browser.

**Concepts practiced:**

- Controlled form inputs
- Lifting state up and passing callback props
- Immutable array updates with `map` and `filter`
- Sorting a copy of an array
- Derived statistics and conditional rendering
- TypeScript types for items and component props
- Browser persistence with `localStorage`

**Technologies:** React, TypeScript, Vite, CSS

[Source Code](./U-RE_Far-Away/) · [Live Demo](https://alexplokhikh.github.io/Educational_Projects/U-RE_Far-Away/)
...
| --- | --- | --- |
| Far Away | Static files generated by Vite | [Live Demo](https://alexplokhikh.github.io/Educational_Projects/U-RE_Far-Away/) |
...
1. Installs dependencies and builds both React apps: Flashcards and Far Away.
2. Copies the root homepage and the two static frontend projects into a shared website folder.
3. Adds each React build under its matching folder: `U-RE_Flashcards/` and `U-RE_Far-Away/`.
